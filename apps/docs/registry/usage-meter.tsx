...
export function UsageMeter({ className, ...props }: UsageMeterPrimitiveProps) {
  return (
    <UsageMeterPrimitive
      className={cn(
        "flex items-center gap-3 rounded-base border border-border bg-surface px-3 py-2",
        className,
      )}
      {...props}
    >
      <UsageMeterTokens className="font-mono text-xs tabular-nums text-muted-foreground" />

      <UsageMeterContext className="group relative h-1 flex-1 overflow-hidden rounded-full bg-surface-muted">
      ...
      <UsageMeterCost className="font-mono text-xs tabular-nums text-foreground" />
    </UsageMeterPrimitive>
  );
}
